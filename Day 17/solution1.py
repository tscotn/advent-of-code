...
# The cdv instruction (opcode 7) works exactly like the adv instruction
    ...
    def sol2(self):
        ...
        while not done:
            for start in check[checking]:
                for i in range(start*8, (start+1)*8):
                    self.A = i
                    self.output = []
                    self.run()
                    # print(self.output[0], self.Program[-(checking+1)])
                    if int(self.output[0]) == self.Program[-(checking+1)]:
                        print('found', i, self.output)
                        check[checking+1] = check.get(checking+1,[]) + [i]
                    # print(i, self.output, self.A, checking)
                    print(check)
            checking+=1
            if checking > len(self.Program):
                done = True
        print(check[len(self.Program)])
